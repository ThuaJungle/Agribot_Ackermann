...
class MotorDriver(Node):
    ...
    def __init__(self):
        ...
        try:
            ...
            # Đợi Arduino khởi động xong (Homing)
            self.get_logger().info('Connected. Waiting 3 seconds for Arduino reset & Homing...')
            time.sleep(3.0) 
    ...
    # Hàm gửi lệnh lái Servo (Ackermann)
    def send_servo_command(self, servo_id, angle):
        # servo_id: 0 là hệ thống lái
        # angle: Góc lái
        self.send_command(f"s {int(servo_id)} {int(angle)}")
    ...
    def motor_command_callback(self, motor_command):
        if (motor_command.is_pwm):
            ...
            # counts per loop = req rads/sec X revs/rad X counts/rev X secs/loor
            # p 
    ...
    def steer_command_callback(self, msg):
        angle = msg.data
        # Gửi lệnh lái xuống Arduino (s 0 <angle>)
        self.send_servo_command(0, angle)

    def check_encoders(self):
        resp = self.send_encoder_read_command()
        if (resp and len(resp) >= 2): # Đảm bảo nhận đủ dữ liệu
    ...
    def send_command(self, cmd_string):
        self.mutex.acquire()
        try:
            cmd_string += "\r" # Thêm ký tự kết thúc lệnh
            try:
            ...
            c = ''
            value = ''
            # Đọc từng ký tự cho đến khi gặp \r
            while c != '\r':
                try:
                ...
                if (c == ''):
                    print("Error: Serial timeout on command: " + cmd_string.strip())
                    return ''
                value += c
...
def main(args=None):
    ...
    # Sử dụng Loop Rate từ tham số
    loop_rate = motor_driver.get_parameter('loop_rate').value
    if loop_rate <= 0: loop_rate = 30 # Giá trị mặc định
        
    rate = motor_driver.create_rate(loop_rate)
    
    while rclpy.ok():
        rclpy.spin_once(motor_driver)
        motor_driver.check_encoders()
        # rate.sleep() # rclpy.spin_once đã quản lý thời gian, nhưng thêm sleep nếu cần
